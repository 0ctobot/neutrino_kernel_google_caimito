...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	/* Don't check if boot completed */
	if (susfs_is_boot_completed_triggered) {
		goto orig_flow;
	}

	/* Only check for KSU process */
				...
				int recurse)
{
	...
	/* Automatically umount bind mounted paths in ksu domain */
	if (!susfs_is_boot_completed_triggered &&
...
#ifdef CONFIG_KSU_SUSFS_AUTO_ADD_SUS_KSU_DEFAULT_MOUNT
	/* KSU magic mount compatibility */
	if (!susfs_is_boot_completed_triggered && !ret &&
