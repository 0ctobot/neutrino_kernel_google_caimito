...
int ksu_handle_setresuid(uid_t ruid, uid_t euid, uid_t suid){
    ...
    // we are only interested in zygote-spawned processes
    if (!susfs_is_sid_equal(current_cred()->security, susfs_zygote_sid)) {
        return 0;
    }

#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
    // force umount if spawned process is isolated service
    if (is_zygote_isolated_service_uid(new_uid)) {
        goto do_umount;
    }
#endif

    // we need to explicitly check if new_uid belongs to ksu manager as it's excluded
    // from allow_list_arr, thus ksu_uid_should_umount will always return true
    if (ksu_get_manager_appid() == new_uid % PER_USER_RANGE) {
        spin_lock_irq(&current->sighand->siglock);
        ksu_seccomp_allow_cache(current->seccomp.filter, __NR_reboot);
        spin_unlock_irq(&current->sighand->siglock);
    ...
    // check if spawned process is user app and needs to be umounted
...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
    // reorder the mnt_id after all sus mounts are umounted
